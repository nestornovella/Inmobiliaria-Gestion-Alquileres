...
module.exports = {
    ...
    getOneLessor: async (req, res, next) => {
        const { email } = req.params
        try {
            if(!email)errorHandler(httpStatusCode.badRequest, 'se requiere el email del locador')
            const lessor = await Lessor.findByPk(email)
            console.log(lessor)
            if(!lessor)errorHandler(httpStatusCode.noEncontrado, 'el locador no se encontro en la base de datos')
            resHandler(httpStatusCode.aceptado, lessor)
        } catch (error) {
            next(error)
        }
    },
